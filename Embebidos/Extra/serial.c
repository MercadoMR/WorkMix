...
main(int argc, char *argv[])
{
	...
	int fd_serie,fp;
        ...
        openlog( "ubicacion.log", LOG_NDELAY | LOG_PID, LOG_LOCAL0 );   
        while(TRUE)
        {
          ubicacion = obtener_posicion(fd_serie);
          if(ubicacion != NULL)
          {
            syslog(LOG_INFO, "Posicion leida! Enviando %s", ubicacion );
	    write(fd_socket,ubicacion+1,sizeof(char)*MAX_BUF);	
          }
        }
